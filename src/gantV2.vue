  ...
  <div class="chart" ref="chart">
    <div class="header">
      <div class="header-left">
        <el-button type="primary" size="mini" @click="handlerAddGantt" style="display:none"
          >新建</el-button
        >
        <el-button type="primary" size="mini" @click="handlerSaveData" style="display:none"
          >保存</el-button
        >
   ...
   <div class="block" style="position: absolute; bottom: 0%;">
    ...
    <div class="date" :style="{ left: rightLineX + 2 + 'px' }">
      ...
      <!-- 甘特图的slider表格外框 何胜金新增            v-if="l.sameIndex == '1'"-->
      <div v-for="(l, index) in listArr"
           :key="index + 'ss'"
           :style="{ 
      ...
      <template v-for="(item, index) in computedList">
         <!-- top: item.top - 19 + 'px', && item.same == 1 && item.sameIndex == 1-->
        <div
          v-if="item.delete == '0'"
          class="line"
          :style="{
...
export default {
  ...
  computed: {
    computedList() {
      ...
      this.list.forEach((item,index) => {
        ...
        //【保证sameIndex从小到大】
        if(item.same == 1 && item.sameIndex == 1){
          this.listArr.push(item);
        }

        //树结构
        if (item.same == 1) {
          if(this.listMapTree.get(item.carId)){
            this.listMapTree.get(item.carId).sameArr.push(item);
          }else{
            item.sameArr.push(item);
            this.listMapTree.set(item.carId,item);
          }
        }else{
          this.listMapTree.set(item.carId,item);
        }
      ...
      this.listArr.forEach(function (itemOuter,indexOuter) {
        //value和key就是map的key，value，map是map本身
         itemOuter.top = (indexOuter > 0 ? ( 33 * (indexOuter + 1) + 7 * (indexOuter)) : (33 * (indexOuter + 1)))
         itemOuter.sameArr.forEach(function (itemInner,indexInner) {
               itemInner.top = itemOuter.top;
         });
      });
  ...
  methods: {
      ...
      let l = [
         {
          ...
          endTime: new Date().getTime() + 1 * 24 * 60 * 60 * 1000,
          id: 4,
          carId:456,
          same:1,
          sameIndex:1,
    ...
    handleEditSave(row) {
      this.list.findIndex(item => {
          if(item.id == this.editRow[0]){
            item.delete = '0';
            item.startTime = item.planTime[0];
            item.endTime = item.planTime[1];
          }
      });
      // console.log(row);
      console.log(this.editRow[0]);//当前行id
      console.log(this.list);
    ...
    //滑动进度条事件
    thunkMousemove() {
      console.log('======thunkMousemove======');
      this.isShowMsg = false;
    },
    //滑动进度条事件
    thunkMousedown() {
      console.log('======thunkMousedown======');
      this.isShowMsg = false;
    },
    //滑动进度条事件
    thunkMouseup(per, e, id, parentId) {
      console.log('======thunkMouseup======');
